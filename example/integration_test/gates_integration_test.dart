...
void main() {
  ...
  group('Detection gates on a real multi-face image', () {
    test(
      'baseline detects multiple faces and widthFraction stays in [0,1]',
      () async {
    ...
    test(
      'minFaceSize keeps exactly the faces at or above the threshold',
      () async {
        ...
        final int expectedKept = baseline
            .where((f) => f.widthFraction >= threshold)
            .length;
    ...
    test('minScore keeps exactly the faces at or above the threshold', () async {
      final bytes = await loadBytes(groupShot);
    ...
    test(
      'zero gates return the full baseline; extreme gates return none',
      () async {
        final bytes = await loadBytes(groupShot);
  ...
  group('Face-presence gate (minFacePresenceConfidence)', () {
    test('defaults to 0.5, matching MediaPipe', () async {
      ...
      expect(
        ...
        0.5,
        reason:
            'the documented MediaPipe-parity default is 0.5, and '
            'changing it silently changes which faces callers get back',
      );
    });

    test(
      'keeps exactly the faces whose meshScore clears the threshold',
      () async {
        final bytes = await loadBytes(groupShot);
        ...
        final baseline = await baseDet.detectFacesFromBytes(
          bytes,
          mode: meshMode,
        );
        await baseDet.dispose();
        ...
        for (final double threshold in <double>[0.25, 0.5, 0.75, 1.0]) {
          ...
          final gated = await gatedDet.detectFacesFromBytes(
            bytes,
            mode: meshMode,
          );
          await gatedDet.dispose();
          ...
          expect(
            ...
            expectedKept,
            reason:
                'threshold $threshold should keep exactly the baseline '
          ...
          expect(
            gated.length,
            lessThanOrEqualTo(previousKept),
        ...
        expect(
          ...
          isTrue,
          reason:
              'no threshold in the sweep filtered anything, so this test '
