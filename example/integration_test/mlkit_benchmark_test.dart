...
void _printLandmarkComparison(Face tfliteFace, mlkit.Face mlkitFace) {
...
List<(Face, mlkit.Face)> _matchFaces(
  ...
  final sortedMlkit = List<mlkit.Face>.from(
    mlkitFaces,
  ...
  return [for (int i = 0; i < count; i++) (sortedTflite[i], sortedMlkit[i])];
}
    ...
    () {
      ...
      for (final imagePath in _kImages) {
        ...
        test('Benchmark: $imageName', () async {
          ...
          final mlkitResult = await _benchmarkMlkit(
            mlkitDetector,
            bytes,
            imageName,
          );
          ...
          final tfliteFaces = await tfliteDetector.detectFacesFromBytes(
            bytes,
            mode: FaceDetectionMode.fast,
          );
