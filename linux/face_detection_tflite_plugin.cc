...
struct _FaceDetectionTflitePlugin {
  GObject parent_instance;
};

G_DEFINE_TYPE(FaceDetectionTflitePlugin, face_detection_tflite_plugin, g_object_get_type())

// Called when a method call is received from Flutter.
static void face_detection_tflite_plugin_handle_method_call(
    FaceDetectionTflitePlugin* self,
...
static void face_detection_tflite_plugin_class_init(FaceDetectionTflitePluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = face_detection_tflite_plugin_dispose;
}

static void face_detection_tflite_plugin_init(FaceDetectionTflitePlugin* self) {}

static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  FaceDetectionTflitePlugin* plugin = FACE_DETECTION_TFLITE_PLUGIN(user_data);
...
void face_detection_tflite_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  FaceDetectionTflitePlugin* plugin = FACE_DETECTION_TFLITE_PLUGIN(
